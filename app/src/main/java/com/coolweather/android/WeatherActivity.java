...
public class WeatherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (weatherString != null){
            ...
            weatherLayout.setVisibility(View.INVISIBLE);//隐藏该控件
            requestWeather(weatherId);
        }
        swipeRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {//触发下拉刷新时
            @Override
        ...
        if (Build.VERSION.SDK_INT >= 21){//系统版本号大于或等于21，也就是5.0以上的系统
            View decorView = getWindow().getDecorView();
            decorView.setSystemUiVisibility(//改变系统UI的显示
                    View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);//表示布局会延伸显示到状态栏上面
            getWindow().setStatusBarColor(Color.TRANSPARENT);//将状态栏设置为透明色
        }

        //设置背景
        String bingPic = prefs.getString("bing_pic",null);//获取缓存数据
        if (bingPic != null){//若有缓存，直接设置背景图
            Glide.with(this).load(bingPic).into(bingPicImg);
        }else {
            loadBingPic();//向服务器获取图片
        }
    }
    ...
    private void showWeatherInfo(Weather weather){
        ...
        weatherLayout.setVisibility(View.VISIBLE);//显示
    ...
    private void loadBingPic(){
        ...
        HttpUtil.sendOkHttpRequest(requestBingPic, new Callback() {//获取数据
            @Override
            public void onFailure(Call call, IOException e) {
                e.printStackTrace();
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                SharedPreferences.Editor editor = PreferenceManager.
                        getDefaultSharedPreferences(WeatherActivity.this).edit();//缓存起来
                editor.putString("bing_pic", bingPic);
                editor.apply();
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.with(WeatherActivity.this).load(bingPic).into(bingPicImg);//设置图片
                    }
                });
            }
        });
